...
def _compute_cell_estimate(props):
    ...
    if est > HARD_LIMIT:
        ...
        safe_base = dx * ((est / HARD_LIMIT) ** (1.0/3.0))
...
def update_cell_estimate(self, context):
    ...
    self.est_cell_count = min(est, 2147483647)
...
def update_courant_and_estimate(self, context):
    """Combined update for subproperties: Courant + live cell estimate. Must return None."""
    update_courant(self, context)
    update_cell_estimate(self, context)


def update_courant_and_estimate_from_subproperty(self, context):
    """Triggers both Courant and Cell Estimate updates from sub-properties."""
    if hasattr(context, "scene") and hasattr(context.scene, "cfmesh_props"):
...
def update_courant(self, context):
    ...
    self.courant_dx_min = dx_min  # used in UI for display only
...
class CFMeshPatch(bpy.types.PropertyGroup):
    ...
    use_local_cell_size: bpy.props.BoolProperty(name="Override Size", default=False, update=update_courant_and_estimate_from_subproperty)
    local_cell_size: bpy.props.FloatProperty(name="Cell Size", default=0.1, min=0.001, subtype='DISTANCE', update=update_courant_and_estimate_from_subproperty)
    boundary_layers: bpy.props.IntProperty(name="Boundary Layers", default=3, min=0, update=update_courant_and_estimate_from_subproperty)
...
class CFMeshBoxRefinement(bpy.types.PropertyGroup):
    ...
    cell_size: bpy.props.FloatProperty(name="Cell Size", default=0.05, min=0.0001, precision=4, subtype='DISTANCE', update=update_courant_and_estimate_from_subproperty)
...
class CFMeshSurfaceRefinement(bpy.types.PropertyGroup):
    ...
    cell_size: bpy.props.FloatProperty(name="Cell Size", default=0.01, min=0.0001, precision=4, subtype='DISTANCE', update=update_courant_and_estimate_from_subproperty)
...
class CFMeshProperties(bpy.types.PropertyGroup):
    ...
    boundary_layers: bpy.props.IntProperty(
        ...
        min=0,
        max=20,
        update=update_courant_and_estimate
    )
    ...
    courant_dx_min: bpy.props.FloatProperty(
        name="Min Cell Size Driving Courant",
        description="Smallest cell size driving the Courant number calculations",
        default=0.1
    )
    ...
    checkmesh_fail_details: bpy.props.StringProperty(name="Failure Details", default="")
