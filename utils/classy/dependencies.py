...
def check_python_deps():
    """Returns (is_installed, error_msg) if required python packages are installed."""
    try:
        import sys
        import os
        # Add local 'libs' folder to sys.path so we can import packages installed there
...
class CLASSY_OT_install_python_deps(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        # Define target directory inside the addon
        addon_dir = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
        libs_dir = os.path.join(addon_dir, "libs")
        
        # Get correct Python executable. sys.executable points to blender.exe on Windows
        python_exe = sys.executable
        if "blender.exe" in python_exe.lower():
            # Construct path to python.exe relative to blender.exe
            blender_dir = os.path.dirname(python_exe)
            version_dir = f"{bpy.app.version[0]}.{bpy.app.version[1]}"
            python_exe = os.path.join(blender_dir, version_dir, "python", "bin", "python.exe")
            if not os.path.exists(python_exe):
                # Fallback just in case
                python_exe = os.path.join(blender_dir, "python", "bin", "python.exe")
