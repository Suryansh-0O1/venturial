...
# --- Caching Helpers for UI Performance (KISS) ---
_cached_geom_obj = None
_cached_geom_type = (False, False, False) # (is_box, is_sphere, is_cylinder)

def get_geometry_classification(obj):
    global _cached_geom_obj, _cached_geom_type
    if not obj or obj.type != 'MESH':
        return False, False, False
    if _cached_geom_obj == obj:
        return _cached_geom_type
    ...
    try:
        ...
        if len(dims) == 3 and dims[0] > 0:
            ...
            if not is_box and num_polygons > 0:
                verts = mesh.vertices
                n_verts = len(verts)
                if n_verts > 0:
                    # Subsample vertices for fast estimation on large meshes
                    step = max(1, n_verts // 1000)
                    sampled_verts = verts[::step]
                    n_sampled = len(sampled_verts)
                    
                    avg_x = sum(v.co.x for v in sampled_verts) / n_sampled
                    avg_y = sum(v.co.y for v in sampled_verts) / n_sampled
                    avg_z = sum(v.co.z for v in sampled_verts) / n_sampled
                    
                    dists = [ ((v.co.x - avg_x)**2 + (v.co.y - avg_y)**2 + (v.co.z - avg_z)**2)**0.5 for v in sampled_verts ]
                    min_dist = min(dists)
                    max_dist = max(dists)
    ...
    _cached_geom_obj = obj
    _cached_geom_type = (is_box, is_sphere, is_cylinder)
    return _cached_geom_type

_last_frame_count_check = 0.0
_cached_frame_count = 0

def get_animation_frame_count(props):
    global _last_frame_count_check, _cached_frame_count
    now = time.time()
    if now - _last_frame_count_check < 2.0:
        return _cached_frame_count
    
    _last_frame_count_check = now
...
class VIEW3D_PT_MeshSettings(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        is_box_geometry, is_sphere_geometry, is_cylinder_geometry = get_geometry_classification(context.active_object)
                            
        # Import STL button at the top
        layout.operator("cfmesh.import_stl_geometry", icon='IMPORT', text="Import STL Geometry")
        layout.operator("cfmesh.refresh_patches", icon='FILE_REFRESH', text="Load Selected as Patches")
        ...
        layout.operator("cfmesh.open_export_dir", icon='FILE_FOLDER', text="Open Export Directory")
        ...
        if props.boundary_layers > 0:
            ...
            if props.boundary_layers > 10:
                warn_box = box.box()
                warn_box.alert = True
                warn_box.label(text="High boundary layer count (>10) may cause mesh quality issues.", icon='ERROR')
            if props.layer_thickness <= 1.0:
                warn_box = box.box()
                warn_box.alert = True
                warn_box.label(text="Thickness ratio should be > 1.0 for boundary layer expansion.", icon='ERROR')
        ...
        row = box2.row()
        row.operator("cfmesh.add_box_refinement", text="Add Box", icon='ADD')
        row.operator("cfmesh.remove_box_refinement", text="Remove Box", icon='REMOVE')

        # Wake box button with tooltip about expected dimensions
        wake_row = box2.row(align=True)
        wake_row.operator("cfmesh.add_wake_preset", text="Auto-Generate Wake Box", icon='MOD_FLUIDSIM')
        ...
        row = box3.row()
        row.operator("cfmesh.add_surface_refinement", text="Add Surface", icon='ADD')
        row.operator("cfmesh.remove_surface_refinement", text="Remove Surface", icon='REMOVE')
        ...
        row = box4.row()
        row.operator("cfmesh.add_cylinder_refinement", text="Add Cylinder", icon='ADD')
        row.operator("cfmesh.remove_cylinder_refinement", text="Remove Cylinder", icon='REMOVE')

        cyl_wake_row = box4.row()
        cyl_wake_row.enabled = not is_box_geometry
        cyl_wake_row.operator("cfmesh.add_cylinder_wake_preset", text="Auto-Generate Wake Cylinder", icon='MOD_FLUIDSIM')
        if is_box_geometry:
        ...
        layout.operator("cfmesh.generate_cfmesh", icon='MESH_CUBE')
...
class VIEW3D_PT_SolverSettings(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        row = layout.row()
        row.scale_y = 1.2
        row.operator("cfmesh.run_solver", icon='PLAY')
...
class VIEW3D_PT_PostProcess(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        layout.operator("cfmesh.run_checkmesh", icon='CHECKMARK')
        if props.checkmesh_result != "Not checked":
            qbox = layout.box()
            qbox.enabled = False
            
            if props.checkmesh_result == "PASSED":
                qbox.label(text="Mesh Quality: PASSED", icon='CHECKMARK')
            elif props.checkmesh_result == "FAILED":
                qbox.label(text="Mesh Quality: FAILED", icon='ERROR')
                if props.checkmesh_fail_details:
                    fail_col = qbox.column(align=True)
                    for detail in props.checkmesh_fail_details.split('; '):
                        if detail:
                            fail_col.label(text=f"• {detail}", icon='DOT')
                    
                    sbox = qbox.box()
                    sbox.label(text="Troubleshooting suggestions:", icon='QUESTION')
                    details_lower = props.checkmesh_fail_details.lower()
                    if 'face tets' in details_lower or 'negative' in details_lower or 'volume' in details_lower:
                        sbox.label(text="  → Reduce Boundary Layers or Thickness Ratio")
                        sbox.label(text="  → Increase Cell Sizes in thin/tight gaps")
                    if 'ortho' in details_lower:
                        sbox.label(text="  → Check that 'Improve Mesh Quality' is enabled")
                        sbox.label(text="  → Ensure smoother cell size transitions")
                    if 'skew' in details_lower:
                        sbox.label(text="  → Check for tiny or intersecting geometry faces")
                        sbox.label(text="  → Use local Box Refinement over high-curvature spots")
            else:
        ...
        if props.inspect_use_bbox:
            ibox.operator("cfmesh.set_inspect_bbox", icon='UV_SYNC_SELECT')
            col = ibox.column(align=True)
            col.prop(props, "inspect_bbox_min")
            col.prop(props, "inspect_bbox_max")
            ibox.operator("cfmesh.inspect_region", icon='ZOOM_SELECTED')
        ...
        # --- Solver Residuals ---
        layout.operator("cfmesh.show_residuals", icon='FCURVE')
        ...
        if props.animate_results:
            n_frames = get_animation_frame_count(props)
            fc_row = vbox.row()
        ...
        vbox.operator("cfmesh.color_by_field", text="Color Boundary Mesh", icon='BRUSH_DATA')
        ...
        sbox.operator("cfmesh.visualize_slice", text="Extract Slice Plane", icon='MESH_GRID')
        
        layout.separator()
        
        # --- Existing buttons ---
        layout.operator("cfmesh.load_result", icon='IMPORT')
        layout.operator("cfmesh.launch_paraview", icon='OUTLINER_OB_FORCE_FIELD')
