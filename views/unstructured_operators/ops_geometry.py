...
class CFMESH_OT_RefreshPatches(bpy.types.Operator):
    bl_idname = "cfmesh.refresh_patches"
    ...
    def execute(self, context):
        ...
        from venturial.models.unstructured_properties import update_courant, update_cell_estimate
        update_courant(props, context)
        update_cell_estimate(props, context)
        return {'FINISHED'}

class CFMESH_OT_AddBoxRefinement(bpy.types.Operator):
    bl_idname = "cfmesh.add_box_refinement"
...
class CFMESH_OT_RemoveBoxRefinement(bpy.types.Operator):
    bl_idname = "cfmesh.remove_box_refinement"
...
class CFMESH_OT_AddSurfaceRefinement(bpy.types.Operator):
    bl_idname = "cfmesh.add_surface_refinement"
...
class CFMESH_OT_RemoveSurfaceRefinement(bpy.types.Operator):
    bl_idname = "cfmesh.remove_surface_refinement"
...
class CFMESH_OT_AddCylinderRefinement(bpy.types.Operator):
    bl_idname = "cfmesh.add_cylinder_refinement"
...
class CFMESH_OT_RemoveCylinderRefinement(bpy.types.Operator):
    bl_idname = "cfmesh.remove_cylinder_refinement"
...
class CFMESH_OT_AddWakePreset(bpy.types.Operator):
    bl_idname = "cfmesh.add_wake_preset"
    ...
    def execute(self, context):
        ...
        min_b_list = (min(xs), min(ys), min(zs))
        max_b_list = (max(xs), max(ys), max(zs))
        vel_norm_list = (vel_norm.x, vel_norm.y, vel_norm.z)
        obj_lens = (obj_len_x, obj_len_y, obj_len_z)
        
        min_bounds, max_bounds = utils_mesh.compute_wake_box_bounds(
            min_b_list, max_b_list, vel_norm_list, obj_lens, 
            3.0, 0.05, mat_flat
        )
        
        item.min_bounds = min_bounds
        item.max_bounds = max_bounds
...
class CFMESH_OT_AddCylinderWakePreset(bpy.types.Operator):
    bl_idname = "cfmesh.add_cylinder_wake_preset"
    ...
    def execute(self, context):
        ...
        p1, p2, final_radius = utils_mesh.compute_wake_cylinder_points(
            min_b_list, max_b_list, vel_norm_list, obj_lens, 
            3.0, avg_scale, mat_flat
        )
        
        item.p1 = p1
        item.p2 = p2
        
        item.radius = final_radius
        item.cell_size = props.base_cell_size * 0.5
...
class CFMESH_OT_ImportSTL(bpy.types.Operator):
    bl_idname = "cfmesh.import_stl_geometry"
